...
const InputSources: React.FC = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'connected': return <CheckCircle className="status-icon status-icon--success" size={16} />
      case 'syncing': return <div className="spinner" />
      case 'error': return <AlertCircle className="status-icon status-icon--error" size={16} />
      default: return <AlertCircle className="status-icon status-icon--warning" size={16} />
    }
  }
  ...
  return (
    ...
    >
      ...
      {/* Connected Sources */}
      <motion.section className="input-sources__section" variants={itemVariants}>
      ...
      <motion.section className="input-sources__section" variants={itemVariants}>
        ...
        <div className="integrations-grid">
          {[
            { name: 'Bank of America', type: 'bank', icon: CreditCard, status: 'available' },
            { name: 'Wells Fargo', type: 'bank', icon: CreditCard, status: 'available' },
            { name: 'PayPal', type: 'payment', icon: CreditCard, status: 'available' },
            { name: 'Venmo', type: 'payment', icon: Smartphone, status: 'available' },
            { name: 'Apple Pay', type: 'payment', icon: Smartphone, status: 'coming-soon' },
            { name: 'Google Pay', type: 'payment', icon: Smartphone, status: 'coming-soon' },
            ...
            <div key={index} className="integration-card">
              ...
              <button 
                className={`btn btn--sm ${
                  integration.status === 'available' ? 'btn--primary' : 'btn--secondary'
                }`}
            ...
            <div key={activity.id} className="activity-item">
              <div className="activity-item__status">
                {activity.status === 'completed' ? (
                  ...
                  <AlertCircle className="status-icon status-icon--error" size={20} />
                )}
              </div>
              <div className="activity-item__content">
                <div className="activity-item__main">
                  <span className="activity-item__action">{activity.action}</span>
                  <span className="activity-item__source">from {activity.source}</span>
                </div>
